...
namespace SimbaForex2
{
    public static class OandaConn
    {
        ...
        public static CandlesResponse LiveDataCaptureNMany(Tuple<string,string,string> args, string instrument, int count)
        {
            string url = args.Item1;
            string accountId = args.Item2;
            string accessToken = args.Item3;
            var request = url + v3Instruments + instrument + String.Format("/candles?count={0}&granularity=M2&price=M",count);
            var response = MakeRequest(request, new object[] { "GET", accessToken });
            bool success = true;
            if (Boolean.TryParse(response.ToString(), out success))
                return null;
