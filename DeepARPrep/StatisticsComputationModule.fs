...
let average_delta(lstX:List<DateTime*float>) = 
    let lst = lstX |> List.where(fun (dte,va) -> va <> 0.)
    if lst.Length < 2 then 
        0.
    else
        let foo = ([for x in [0..lst.Length - 2] do yield (((snd lst.[x+1]) - (snd lst.[x])) / (snd lst.[x]))] |> List.sum) / float(lst.Length - 1)
        if Double.IsInfinity(foo) then 
...
let deviation_delta(lst:List<DateTime*float>,average:float) = 
    if lst.Length < 2 then 
        ...
        let va = Math.Sqrt(([for x in [0..lst.Length - 2] do yield Math.Pow((((((snd lst.[x+1]) - (snd lst.[x])) / (snd lst.[x]))) - average),2.)] |> List.sum) / (float(lst.Length - 2)))
        va
let correlation_delta(lst:List<DateTime*float>,lst2:List<DateTime*float>,avgS:string,avg2S:string,devS:string,dev2S:string) = 
    ...
    let mutable dev = 0.
    let mutable dev2 = 0.
    if Double.TryParse(avgS,&avg) && Double.TryParse(avg2S, &avg2) && Double.TryParse(devS, &dev) && Double.TryParse(dev2S, &dev2) then 
    
        let ValueFor(date,lst)= 
            let va = snd (lst |> List.find(fun (dte,gam) -> dte = date))
            va
        if lst.Length < 2 || lst2.Length < 2 then 
            0.
        elif ([avg;avg2;dev;dev2] |> List.exists(fun chi -> chi = 0.)) = false then
            if (lst |> List.where(fun (dte,gam) -> gam <> 0.) |> List.exists(fun (dte,gam) -> (lst2 |> List.where(fun (dte2,rho) -> rho <> 0.) |> List.collect(fun (dte2,rho) -> [dte2]) |> List.contains(dte)))) then 
                let dtes = (lst |> List.where(fun (dte,gam) -> gam <> 0.)  |> List.where(fun (dte, gam) -> (lst2 |> List.where(fun (dte2,rho) -> rho <> 0.) |> List.collect(fun (dte2,rho) -> [dte2]) |> List.contains(dte))) |> List.collect(fun (dte,gam) -> [dte]))
                if dtes.Length > 2 then 
                    let va = ([for x in [0..dtes.Length - 2] do yield (((ValueFor(dtes.[x+1],lst) - ValueFor(dtes.[x],lst)) / ValueFor(dtes.[x],lst)) - avg) * (((ValueFor(dtes.[x+1],lst2) - ValueFor(dtes.[x],lst2)) / ValueFor(dtes.[x],lst2)) - avg2)] |> List.sum) / (float(lst.Length - 2) * dev * dev2)
                    va
                else
                    0.
            else
...
type DeviationsComputation(date:DateTime) = 
    ...
    do
        while ContinueIteration(ctr,whole_file.Length) do
            let averages_row = [for st in (averages_file.[1..averages_file.Length - 1] |> List.find(fun row -> row.[0] = whole_file.[ctr + BackDataLength].[0])) do yield st]
            ...
            if (dev_fles |> List.exists(fun fle -> fle.Name = date.Year.ToString() + "_" + date.Month.ToString() + ".csv")) then 
                ...
                File.AppendAllText(deviations_data + String.Format(@"\Backward\{0}",FileName(date)),"\n" + whole_file.[System.Math.Min(ctr + BackDataLength, whole_file.Length - 1)].[0]+","+String.Join(",",deviations))
            let fw_averages_row = [for st in (forward_averages_file.[1..forward_averages_file.Length - 1] |> List.find(fun row -> row.[0] = whole_file.[ctr + BackDataLength].[0])) do yield st]
...
type CorrelationsComputation(date:DateTime) = 
    ...
    let deviation_file = [for fle in (new DirectoryInfo(deviations_data + @"\Backward")).GetFiles() do yield fle] |> List.find(fun fle -> fle.Name = FileName(date))
    let forward_deviation_file = [for fle in (new DirectoryInfo(deviations_data + @"\Forward")).GetFiles() do yield fle] |> List.find(fun fle -> fle.Name = FileName(date))
    let whole_file = ReadAndParseFile(file)
    let exchanges = whole_file.[0]
    ...
    do
        while ContinueIteration(ctr,whole_file.Length) do
            ...
            if (corr_fles |> List.exists(fun fle -> fle.Name = FileName(date))) then 
                ...
                File.WriteAllText(correlations_data + String.Format(@"\Backward\{0}",FileName(date)),exchanges.[0] + "," + String.Join(",",Square(exchanges.[1..exchanges.Length - 1])))
                File.AppendAllText(correlations_data + String.Format(@"\Backward\{0}",FileName(date)),"\n" + whole_file.[System.Math.Min(ctr + BackDataLength, whole_file.Length - 1)].[0]+"," + Elongate(correlations))
